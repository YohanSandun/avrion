...
u8 AvrCpu::exec_adc(u16 opcode) {
    ...
    u8 rr = reg(r);
    u8 rd = reg(d);
    u8 result = rd + rr + ((sreg() & 0x01) ? 1 : 0); // C flag is carry from previous operation
    set_reg(d, result);
...
u8 AvrCpu::exec_sbci(u16 opcode) {
    // SBCI
    // 0100 KKKK dddd KKKK
    u8 d = 16 + ((opcode >> 4) & 0xF);
    u8 k = (opcode & 0x000F) | ((opcode >> 4) & 0xF0);
    u8 rd = reg(d);
    u8 result = rd - k - ((sreg() & 0x01) ? 1 : 0);
    set_reg(d, result);
...
u8 AvrCpu::exec_or(u16 opcode) {
    ...
    _sreg = (result & 0x80) != 0
        ? (_sreg | 0x04) : (_sreg & ~0x04); // N set if bit 7 of result is set
    _sreg = (_sreg & ~0x08u); // V cleared
    // S = N XOR V; since V is always 0 for OR, S = N
    _sreg = (_sreg & 0x04)
        ? (_sreg | 0x10) : (_sreg & ~0x10);
    set_sreg(_sreg);
    return 1;
